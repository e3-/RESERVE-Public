...
def main(files_to_ignore=files_to_ignore, start_date=start_date, end_date=end_date, ML_time_step=ML_time_step,
         model_name=model_name, lag_term_start_predictors=lag_term_start_predictors,
    ...
    raw_data_df = pd.DataFrame(index=pd.date_range(start=start_date, end=end_date, freq=ML_time_step, closed="left"))
    raw_data_validity = raw_data_df.copy()

    # Iterate over each feature output from data-checker
    print("Reading in outputs from data-checker and ensuring temporal resolution meets requirement for model....")
    for file in os.listdir(path_to_data_checker_outputs):
        ...
        if feature_time_step == ML_inputs_time_step:
            raw_data_df, raw_data_validity = merge_df(raw_data_df, feature_df[col_name_for_value],
                                                           raw_data_validity,
            ...
            raw_data_validity = raw_data_validity.rename(columns={col_name_for_validity_flag: feature_name})
        # ==== Option 2 of 3 ====
        ...
        # out of the one 5-min feature currently being assessed
            ...
            assert ML_inputs_time_step % feature_time_step == 0, "ML time-step needs to be equal to or a perfect multiple of " \
                                                                 "feature time-step. Not the case for {}".format(
                feature_name)
            ...
            feature_name_substep_idx = [feature_name + "_" + str(i) for i in range(num_substeps)]
            feature_name_substep_idx = np.tile(feature_name_substep_idx, int(feature_df.shape[0] / num_substeps))
            ...
            feature_df.index = pd.MultiIndex.from_arrays([timestamp_idx, feature_name_substep_idx])
        ...
        # value 4 times, once for each 15 min interval in the given hour
            ...
            assert feature_time_step % ML_inputs_time_step == 0, "Feature time-step needs to be equal to or a perfect " \
                                                                 ...
                                                                 "{}".format(feature_name)
            repeated_feature_df = feature_df.resample(str(ML_inputs_time_step) + "S").pad()
            raw_data_df, raw_data_validity = merge_df(raw_data_df, repeated_feature_df[col_name_for_value],
                                                           raw_data_validity,
